...
academic_collabs_versus_others <- data_without_fmd %>% 
  ...
  mutate(year_aggreg = as.numeric(ifelse(publication_year < 2006, 2005, publication_year
         ...
         collab_type = as_factor(if_else(str_detect(author_affiliation_type, 
...
academic_collabs_trend <- ggplot(data = academic_collabs_versus_others) + 
  ...
  coord_flip() +
  labs(title = 'Proportion of mixed collaborations per year',
       x = 'Year',
...
#' Modify the data frame to lump together all countries with less than some 
#' number of studies as 'other' and recalculate the percentage studied
...
continents_studied_by_count <- countries_studied_by_count %>% 
  ...
  mutate(continent_studied = str_to_lower(continent_studied), 
         continent_studied = case_when(
           str_detect(continent_studied, 'asia') == TRUE ~ 'asia',
           str_detect(continent_studied, 'africa') == TRUE ~ 'africa',
           TRUE ~ continent_studied)
           ) %>% 
  group_by(continent_studied) %>%
  summarise(num_of_studies = sum(num_of_studies), .groups = 'drop') %>% 
  arrange(desc(num_of_studies)) %>% 
  filter(!(continent_studied %in% c('none', 'northern_hemisphere', 'global'))) 

#Print the results
knitr::kable(continents_studied_by_count %>% adorn_totals(), 
             caption = "Number of studies per continent studied")
```
...
knitr::kable(continents_studied,
...
author_location_count_by_disease <- data_without_fmd %>%
  ...
  group_by(disease) %>%
  distinct(title, .keep_all = TRUE) %>% 
  count(author_in_country_studied, sort = TRUE, name = "num_of_studies") %>% 
  complete(author_in_country_studied, fill = list(num_of_studies = 0)) %>% 
  pivot_wider(names_from = author_in_country_studied, values_from = num_of_studies) %>% 
...
knitr::kable(author_location_count_by_disease,
             caption = "Author connection to the location studied by disease")
```
