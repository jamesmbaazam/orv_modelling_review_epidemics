...
# The data with citations 
compact_data_with_citation_keys_init <- read_rds(normalizePath('../data/compact_data_with_citation_keys_cleaned.rds')) 
...
data_without_fmd <- compact_data_with_citation_keys %>% filter(disease != 'fmd') 
```
...
studies_in_2009_2011_df <- data_without_fmd %>%
  separate_rows(disease, sep = ' ') %>% 
...
```{r echo=FALSE}
studies_between_2015_2016_df <- data_without_fmd %>%
  separate_rows(disease, sep = ' ') %>% 
...
```{r echo=FALSE}
studies_in_2019_df <- data_without_fmd %>%
