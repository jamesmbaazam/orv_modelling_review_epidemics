...
if (!dir.exists(here::here(gh_figs_dir))) {
  dir.create(here::here(gh_figs_dir),
    showWarnings = TRUE
  )
}
```
...
The review involved `r nrow(compact_data_with_citation_keys)` published between
`r min(compact_data_with_citation_keys$publication_year)` and `r max(compact_data_with_citation_keys$publication_year)`.

## Breakdown of studies before and after 2005
```{r}
before_2005 <- compact_data_with_citation_keys_no_fmd %>% 
  select(publication_year) %>% 
  mutate(before_2005 = ifelse(publication_year <= 2005, "yes", "no")) %>% 
  count(before_2005, name = "num_of_studies") %>% 
  adorn_percentages(denominator = 'col') %>% 
  adorn_pct_formatting() %>% 
  adorn_ns(position = "front")
  

knitr::kable(before_2005)
```
...
ggsave(filename = "fig2_publications_per_year.png",
       plot = publications_per_year_plot,
       path = dropbox_figs_dir
       )
```

# Human vaccine-preventable diseases

## Analysis of collaboration type compositions
...
### Number of studies per year and broken down by collaboration types
...
collap_types_by_year <- compact_data_with_citation_keys_no_fmd %>% 
  ...
  clean_names() %>% 
  adorn_totals(... = c(mixed, purely_academic), where = c("row", "col"))

knitr::kable(collap_types_by_year,
             caption = "Breakdown of studies per year by collaboration types")
```
...
### Number of publications per year by collaboration type 
...
```{r} 
 ...
 collabs_per_year_plot <- ggplot(
   data = collabs_per_year,
   aes(
     x = year_aggreg,
 ...
 ggsave(
   filename = "total_collabs_per_year_plot.tiff",
   plot = collabs_per_year_plot,
   path = gh_figs_dir
 )
...
Even though non-academic collaborations increased over time, the share of 
total publications per year over time decreased. 

### Proportion of the total publications per year by collaboration type
...
ggsave(
  filename = "collab_proportions.tiff",
  plot = collab_proportions_plot,
  path = gh_figs_dir
)
...
### Mixed collaborations with and without academics

```{r echo=FALSE}
academic_collabs_versus_others <- compact_data_with_citation_keys_no_fmd %>%
  filter(author_affiliation_type != "academic_institutions") %>%
  mutate(
    year_aggreg = as.numeric(ifelse(publication_year < 2006, 2005, publication_year)),
    collab_type = as_factor(if_else(str_detect(
...
academic_collabs_trend <- ggplot(data = academic_collabs_versus_others) +
  ...
  coord_flip() +
  labs(
    title = "Proportion of mixed collaborations per year",
    x = "Year",
    y = "Proportion of publications",
    fill = "Mixed collaboration type"
  ) +
  theme_minimal(base_size = 12)
...
```
### Diseases and interventions (top 2 per disease)

```{r echo=FALSE}
#count the number of unique interventions modelled
interventions_per_disease_top2 <- long_data_no_fmd %>% 
  group_by(title) %>% 
  distinct(intervention_modelled, .keep_all = TRUE) %>% 
  ungroup() %>% 
  count(disease, intervention_modelled, name = 'num_of_studies') %>% 
  arrange(disease, desc(num_of_studies)) %>% 
  group_by(disease) %>% 
  slice_head(n = 2) %>% #keep only top 2 per group 
  ungroup()

knitr::kable(interventions_per_disease_top2,
             caption = "Top 2 most studied intervention per disease")
```
...
```{r}
outcomes_measured_by_collab_type <- compact_data_with_citation_keys_no_fmd %>% 
...
knitr::kable(outcomes_measured_by_collab_type, 
             caption = 'Outcomes studied by the collaboration types', 
             format = "latex", booktabs = TRUE, longtable = TRUE) %>% 
        kableExtra::kable_styling(latex_options = c('striped', 
                                                    'HOLD_position', 
                                                    'repeat_header')
...
### Top 6 outcomes measured by collaboration type
```{r}
outcomes_measured_by_collab_type_top6 <- outcomes_measured_by_collab_type %>% 
  group_map(~ {
    .x %>% 
      adorn_totals() %>% 
      adorn_percentages(denominator = "col") %>%
...
knitr::kable(outcomes_measured_by_collab_type_top6, 
             caption = 'Top 6 outcomes studied by the collaboration types', 
             format = "latex", booktabs = TRUE, longtable = TRUE) %>% 
        kableExtra::kable_styling(latex_options = c('striped', 
                                                    'HOLD_position', 
                                                    'repeat_header')
