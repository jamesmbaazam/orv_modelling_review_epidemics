...
```{r rerun_data_cleaning_if_interactive}
#' if this script is being knit interactively, re-run the data cleaning step,
#' and assign the generated cleaned data (in case something has changed) 
#' else if through the terminal, use the 
#' arguments passed through the commandline/make
# knitting_interactively <- isFALSE(is.null(getOption('knitr.in.progress')))
# 
# if (knitting_interactively) {
#   source(here::here("scripts", "orv_models_review_data_cleaning.R"))
# args <- c(
...
continents_studied_by_count <- countries_studied_by_count %>% 
  ...
  mutate(continent_studied = case_when(str_detect(continent_studied, 'asia') == TRUE ~ 'asia', 
...
author_connectedness_to_location_aggreg <- compact_data_with_citation_keys_no_fmd %>% 
  filter(author_in_country_studied != 'not_applicable') %>% 
  filter(!country_studied %in% c('other', 'none', 'northern_hemisphere', 'west_africa', 'global')) %>% 
