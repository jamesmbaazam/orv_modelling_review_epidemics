...
```{r}
#' if sourced, re-run the data cleaning step, in case something has changed 
#' else if through the terminal, use the arguments passed through the
#' commandline/make
if (is_null(getOption('knitr.in.progress'))) {
...
long_data <- read_rds(args[1])

#The cleaned data without citations
compact_data_init <- read_rds(args[2])

# The data with citations 
compact_data_with_citation_keys <- read_rds(args[3])
